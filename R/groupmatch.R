...
#'
#' @param group Grouping variable for control group.  In the case of rolling
#' enrollment, this will be a unique subject identifier pertaining to all
#' 'copies' or 'versions' of the same subject.
#'
#' @param allow_duplicates When \code{allow_duplicates} is FALSE, the algorithm
...
#' @keywords nonparametric optimize rolling enrollment
#' @export
...
groupmatch.matrix <- function(x, group = NULL, allow_duplicates = FALSE,
  ...
  if((min.controls > 0 & min.controls < 1)){
    stop("argument \'min.controls\' must be equal to 0 or greater than or equal to 1")
  }
  if(max.controls < 1){
    stop("argument \'max.controls\' must be greater than or equal to 1")
  }
  ...
  .groupmatch <- function(d, mnctl, mxctl, omf, g, ad) {
  ...
  .groupmatch.with.recovery <- function(d.r, mnctl.r, mxctl.r, omf.r,
    ...
    if (mxctl.r * dim(d.r)[1] >= prod(denom, 1-omf.r, na.rm=TRUE)) {
      tmp <- .groupmatch(d.r, mnctl.r, mxctl.r, omf.r, g.r, ad.r)
    ...
    if(is.finite(mxctl.r) & mxctl.r >= 1) {
      ...
      tmp2 <- list(.groupmatch(d.r, mnctl.r, Inf, omf.r, g.r, ad.r))
      ...
      return(.groupmatch(d.r, mnctl.r, mxctl.r, new.omf.r, g.r, ad.r))
    } else {
